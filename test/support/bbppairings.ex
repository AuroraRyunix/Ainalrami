defmodule OpenPair.Test.Bbppairings do
  @moduledoc """
  ...
  implementation) locally, purely for comparison-testing OpenPair's own
  ...
  of its SOURCE to port `OpenPair.WeightedMatching`.

  NOT vendored into this repo, same reasoning as `OpenPair.Test.Javafo`:
  ...
  — same sibling-checkout convention `OpenPair.Test.Javafo` already uses).
  ...
  `parse_output/1` is the same logic as `OpenPair.Test.Javafo`'s.
  ...
  also the exact case `OpenPair.Pairing.NoValidPairingError` exists for;
  ...
  `trf_text` and returns:
    ...
    * `{:ok, pairs}` — `[{white_rank, black_rank | nil}]`, same convention
      `OpenPair.Test.Javafo.pair/1` and `OpenPair.Pairing` both use
  ...
  def pair(trf_text) do
    dir = Path.join(System.tmp_dir!(), "openpair-bbp-#{System.unique_integer([:positive])}")
    File.mkdir_p!(dir)
