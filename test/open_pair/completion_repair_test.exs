defmodule OpenPair.CompletionRepairTest do
  @moduledoc """
  ...
  So `OPENPAIR_FORCE_STRAND=1` restores the old, wrong stop condition on
  ...
  test "a round the cascade strands is still paired legally" do
    ...
    assert differed > 0,
           "OPENPAIR_FORCE_STRAND changed nothing across #{paired} rounds, so this proved " <>
  ...
  defp pair_with(players, flag) do
    if flag,
      do: System.put_env("OPENPAIR_FORCE_STRAND", flag),
      else: System.delete_env("OPENPAIR_FORCE_STRAND")

    try do
      Pairing.pair_next_round(players, expected_rounds: @rounds)
    rescue
      OpenPair.Pairing.NoValidPairingError -> :refused
    after
      System.delete_env("OPENPAIR_FORCE_STRAND")
    end
  end
