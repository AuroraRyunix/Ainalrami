defmodule OpenPair.ThreeWayComparisonTest do
  @moduledoc """
  ...
  the same order as the ruler's own slack, and "openpair is wrong" and
  ...
  use ExUnit.Case
  alias OpenPair.{Pairing, Test.Bbppairings, Test.Gacrux}

  @moduletag :three_way
  @moduletag timeout: :infinity

  test "bbpPairings, Gacrux and OpenPair on identical positions" do
  ...
  defp report(rows) do
    ...
    IO.puts("  OpenPair     vs bbpPairings  #{ours_bbp}/#{n}  #{pct.(ours_bbp)}")
    IO.puts("  OpenPair     vs Gacrux       #{ours_gac}/#{n}  #{pct.(ours_gac)}")
    ...
    IO.puts("\n  of the #{length(ours_differs)} round(s) where OpenPair differs from either:")
    IO.puts("    references agree, so OpenPair is the odd one out   #{outvoted}")
  ...
  defp build_trf(players, total_rounds) do
    OpenPair.Trf.serialize(%{tournament: %{name: "ThreeWay", type: "swiss"}, players: players}) <>
      "152 W\r\nXXR #{total_rounds}\r\n"
  end
